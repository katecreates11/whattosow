...
- `docs/ANTI_PATTERNS.md` — **never-do-this/do-this-instead library** (equal grids, chips, metric strips, panels, repeated CTAs, over-explaining, guide grids, product buttons, link farms, AI copy). Check every new/changed block against it before a PR; two matches = redesign it.
...
- `docs/COPY_REWRITES.md` — surface-by-surface bad/better copy pairs (answers, watering notes, verdicts, disclosures, buying advice, nav, footer, empty and error states) + the calibration rule: one image per moment, and it must carry information.
