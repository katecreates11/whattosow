...
The Forager writes a ranked shortlist here each week. **You (Kate) are the gate:** change a
card's `Status` to `approved`, `parked`, or `binned`. Approved content ideas flow to the
Night Gardener to build; approved features get spec'd first; approved monetisation angles are
usually yours to action. Parked and binned ideas are never re-pitched — this board is the
Forager's memory.

## Status key

- **proposed** — the Forager's pitch, awaiting your call.
- **approved** — yes. Eligible for the Night Gardener (content / spec'd feature) or for you to action.
- **parked** — not now, but not a no. Don't re-pitch.
- **binned** — no. Don't re-pitch.

Feature ideas marked `approved` also get a `needs-spec` note until they've been brainstormed
into a proper build item.
...
_Shortlist from the Forager's first run — ranked best-first. Set each `Status` to approved / parked / binned._
...
- **Ready-to-queue brief:** target "autumn planting garlic uk" / "when to order onion sets uk"; angle = plan-and-order-now, not how-to-grow; links ← growing-onions-garlic-leeks, /sow; voice = practical, gently timely ("worth getting your order in"), never hypey; **check Awin has autumn-allium products live at Suttons/Dobies before building.**
...
- **Ready-to-queue brief:** target "sow now for winter harvest" (let /sow/[month] keep "what to sow in august uk"); angle = reassurance + warm-soil hook; links → /sow, succession-sowing, growing-winter-salad-leaves; voice = gentle, weather-obsessed about soil warmth, never scolding.
- **Added:** 2026-07-07 · **Run:** forager-test-2026-07-07
...
- **Ready-to-queue brief:** target "how to save vegetable seeds uk" / "saving tomato/bean/pea seed"; angle = thrift + closing the loop; links → beans/peas/tomatoes crop pages, /sow; voice = satisfied, unfussy; be honest about the F1 caveat.
- **Added:** 2026-07-07 · **Run:** forager-test-2026-07-07
...
_**Recovered by The Dreamer, 25 July** — this run committed to a branch (`ideas/weekly-2026-07-19`) instead of straight to `main` as the Forager's own instructions say it should, so these three sat unseen for six days. Recovered here, unedited, so they finally reach the bench._
...
_Shortlist from The Dreamer's run of 25 July — one small addition, tagged `from-a-dream`. Checked against everything above; this isn't new content, it's a scope addition to an already-approved card, so it gets its own yes rather than being assumed._
...
- **Pitch:** no new page, no new research — just a brief addition to card 12's build: reuse the existing postcode/location plumbing to name the reader's own water company where the data allows it, and say plainly whether they're currently restricted or not, rather than a generic "some areas are restricted" sentence. Everyone without a saved postcode still gets the honest general explainer as pitched.
...
_**Recovered by The Dreamer, 1 August** — this run committed to a branch (`ideas/weekly-2026-07-26`) instead of straight to `main` as the Forager's own instructions say it should, for the second time since the write-to-main fix landed on 12 July. Recovered here, unedited, so they finally reach the bench — see `docs/dreams/wisdom.md` §5 for the pattern this is now confirming._
...
_The Dreamer's run of 1 August — one small addition, tagged `from-a-dream`. Checked against everything above; this isn't new content, it's a scope addition to an already-pitched, not-yet-built card, so it gets its own yes rather than being assumed._
...
- **Pitch:** no new page, no new research — a brief addition to card 13's build: the potato section should explicitly cover the two signals that decide *when* to lift (foliage dying back naturally, not cut early) and *when* it's safe to store (a fortnight left in the ground or in a dry spot afterwards for skins to set), not just how to cure once lifted. Everything else about card 13 stays as pitched.
...
_**Recovered by The Dreamer, 22 August, restated 29 August** — a third
Forager run (16 August) committed to a branch (`ideas/weekly-2026-08-16`)
instead of `main`, the same pipeline bug that's now hit six runs since 19
July. This run branched before last week's recovery landed, so its own card
numbers (22–24) collide with cards already on the board; renumbered here to
30–32. The Dreamer's own recovery branch from 22 August was never merged
either, so these never reached the real board until now — see
`docs/dreams/wisdom.md` §5._
...
- **Note (The Dreamer, 29 August):** the 23 August Forager run independently pitched the same idea ("When to pick your Halloween pumpkin — and how to keep it that way") — the second confirmed duplicate the write-to-main bug has produced. Folded here rather than added twice; this card has the head start.
- **Added:** 2026-08-16 · **Run:** forager-2026-08-16
...
_The Dreamer's run of 22 August, restated 29 August — two small additions,
tagged `from-a-dream`. Both are sourced from the SEO Watchdog's 18 August
report rather than fresh research — that report cross-checked its own
findings against the board before writing them up and confirmed neither
duplicates an existing card._
...
_**Recovered by The Dreamer, 29 August** — a fourth Forager run (23 August)
committed to a branch (`ideas/weekly-2026-08-23`) instead of `main`. Its
third pitch, Halloween-pumpkin timing, duplicated card 31 and is folded into
the note there rather than added again — see `docs/dreams/wisdom.md` §5._

_Shortlist from the Forager's run of 23 August — ranked best-first._
